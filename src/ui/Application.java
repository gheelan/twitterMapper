...
public class Application extends JFrame {
    ...
    private void initialize() {
        // To use the live TwitterMapperPackageDiagram stream, use the following line
        twitterSource = new LiveTwitterSource();

        // To use the recorded TwitterMapperPackageDiagram stream, use the following line
    ...
    /**
     * return a list of all terms mentioned in all queries. The live TwitterMapperPackageDiagram source uses this
    ...
    public Application() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                ICoordinate mousePosition = map().getPosition(mousePoint);
                List<MapMarkerAdvanced> markersAtPos = getMarkersAtPos(mousePosition, pixelWidth(mousePoint));

                for (MapMarkerAdvanced mma : markersAtPos) {
                    MapMarkerRectangle mmr = new MapMarkerRectangle(mma.getLayer(), mma.getCoordinate(), mma.getColor(), mma.getImg(), mma.getStatus());
                    map().addMapMarker(mmr);
                }

                if (markersAtPos.isEmpty()){
                    for (MapMarker m : map().getMapMarkerList()){
                        if (m instanceof MapMarkerRectangle){
    ...
    private List<MapMarkerAdvanced> getMarkersAtPos(ICoordinate pos, double pixelWidth) {
        List<MapMarkerAdvanced> ans = new ArrayList<>();
    ...
    public void terminateQuery(Query query) {
        ...
        query.terminate();
        twitterSource.removeObserver(query);
        Set<String> allterms = getQueryTerms();
        twitterSource.setFilterTerms(allterms);
